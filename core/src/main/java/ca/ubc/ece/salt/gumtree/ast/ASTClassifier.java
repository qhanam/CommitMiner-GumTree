...
public class ASTClassifier {
    ...
    public ASTClassifier(TreeContext src, TreeContext dst, TreeClassifier treeClassifier,
	...
	/*
	 * Classify the children of the classified AST nodes and assign node mappings
	 * for MOVED, UPDATED and UNCHANGED nodes.
	 */
	...
	/*
	 * Iterate through the changed Tree nodes and classify the corresponding AST
	 * node with the Tree node's class.
	 */
	for (ITree tree : set) {
    ...
    /**
     * Propagate the label up to the statement or function level (depending on
     * needs)
     */
    private void propagateChangesToAncestors(ITree subtree) throws InvalidClassException {
	/*
	 * Propagate UPDATED label upwards if this subtree was inserted, removed or
	 * updated.
	 */
     ...
     * 
     * @param subtree
     *            The subtree to label.
     * @param ancenstorChangeType
     *            The change type of the current node's ancestors.
     * @param isSrc
     *            {@code true} if the current tree is the original AST;
     *            {@code false} if the current tree is the new AST.
     * @throws InvalidClassException
     */
    private void propagateChangesToChildren(ITree subtree, ChangeType ancestorChangeType,
	    boolean ancestorMoved, boolean isSrc) throws InvalidClassException {
	ClassifiedASTNode classifiedNode = subtree.getClassifiedASTNode();

	/* This node was moved if its parent was moved. */
	if (ancestorMoved)
	    classifiedNode.setMoved(true);
	...
	if (classifiedNode.getChangeType() == ChangeType.UNCHANGED
		...
		if (dst != null)
		    classifiedNode.map(dst.getClassifiedASTNode());
	    } else {
		ITree src = this.mappings.getSrc(subtree);
		if (src != null)
		    classifiedNode.map(src.getClassifiedASTNode());
	    }
	}

	/* Assign the node a version label. */
	if (isSrc)
	    classifiedNode.setVersion(Version.SOURCE);
	else
	    classifiedNode.setVersion(Version.DESTINATION);

	/* Classify this node's children with the new change type. */
	for (ITree childOfSubtree : subtree.getChildren()) {
	    propagateChangesToChildren(childOfSubtree, classifiedNode.getChangeType(),
		    classifiedNode.getIsMoved(), isSrc);
	}

    }

    /**
     * Visits all the ancestors of the node and labels them as {@code UPDATED} if
     * they have the label {@code UNCHANGED}.
     * 
     * @param child2
     *            The updated/inserted/removed node who's ancestors should be
     *            labeled.
     */
	...
	if (child2.getClassifiedASTNode().isFunction())
	    return;

	/*
	 * Climb the tree and label all unchanged nodes until we get to the statement.
	 */
	...
	while (ancestor != null && ancestor.getClassifiedASTNode() != null
	    ...
	    /*
	     * Label the ancestor, and it's mapped node in the other version, as updated,
	     * since one of its descendants was inserted, removed or updated.
	     */
